...
import InvoiceInfo from '../../components/RoomServicePos/InvoiceInfo/InvoiceInfo';
import GrandTotal from '../../components/RoomServicePos/GrandTotal/GrandTotal';
...
const RoomServicePos = () => {
	...
	const searchCategoryOnChangeHandler = async (searchValue) => {
		...
		if (!_.isUndefined(id)) {
			try {
				let response = await axios.get(`/room-service-categories/${id}`);
				let data = response?.data;
				let reArrangeData =
					!_.isUndefined(data) &&
					data.map((c) => {
	...
	const searchSubCategoryOnChangeHandler = async (product) => {
		if (!_.isUndefined(product?.id)) {
			try {
				let response = await axios.get(`/room-service-products/${product?.id}`);
				let p_info = response?.data?.[0];
				let added_price = Array.isArray(p_info?.room_services) && _.size(p_info?.room_services) > 0 && p_info?.room_services[0]?.room_service_extra_price;
				if (!_.isNil(added_price)) p_info['price'] = parseInt(added_price);
	...
	const onSubmit = (e) => {
		...
		if (!_.isNil(editableProduct)) {
			...
			dispatch({ type: 'ADD_ROOM_INVOICE_DATA', payload: productSelectBundle });
		}
	...
	const getComboProducts = async () => {
		try {
			const response = await axios.get(`/event-type/room`);
			...
			<div className="restaurant position-relative py-4 pb-5">
				<div className="row align-items-center mb-4">
					<div className="col-xl-6">
						<h3 className="mb-0 fw-2 text-primary fw-normal">Room Service POS</h3>
					</div>
					<div className="col-xl-6">
						<div className="shortcut-command d-flex align-items-center justify-content-end">
							...
							<Link to={'/room-service-pos/current-orders'} className="btn btn-info text-light bg-gradient btn-sm px-2 me-2 me-sm-4">
							...
							<div className="combo-product-list rounded mt-2">
								...
								<div className="row text-center gx-1">
									{comboProducts.map((product) => {
										return (
											<div className="col-sm-3 col-xxl-2" key={product?.id}>
												<div
													onClick={() => {
						...
						<InvoiceInfo
							editInvoiceProduct={(editable) => {
								if (!_.isNil(editable)) {
									...
									editable?.event_type === 'room' ? comboPackageClickHandler(editable?.id) : searchSubCategoryOnChangeHandler(editable);
								}
							}}
