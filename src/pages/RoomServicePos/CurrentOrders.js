...
const CurrentOrders = () => {
	...
	const getCurrentOrders = async () => {
		try {
			let response = await axios.get(`/room-service-order-list`);
	...
	const getCurrentOrdersByFilter = async (status) => {
		try {
			let response = await axios.get(`/room-service-order-list/${status}`);
	...
	const editOrder = async (iId) => {
		try {
			...
			dispatch({ type: 'EDIT_ROOM_INVOICE_DATA', payload: { orderData: order, products: reArrangeProducts } });
			setTimeout(() => navigate('/room-service-pos'), 100);
		} catch (error) {
			console.error(error);
		}
	};
												...
												<table className="table text-start table-sm">
													...
													<tfoot className="table-success">
														...
														<tr>
															...
															<td>
																{currency_position === 'prefix' && currency_symbol} {printPreviewContent?.trans_service}{' '}
									...
									<tbody>
										...
										<tr>
											<td className="border-bottom">
												...
												<p className="mb-0">Room Number : {kitchenPrintContent?.hc_room}</p>
												<p className="mb-0">
