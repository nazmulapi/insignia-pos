...
const InvoiceInfo = ({ getTotalInfo, editInvoiceProduct }) => {
  const {
    register,
    handleSubmit,
    reset,
   ...
   */
  const invoiceInfoTableData = useSelector(
    (reduxStore) => reduxStore?.InvoiceInfoTableReducer?.invoiceTableData
  );
  const orderData = useSelector(
    (reduxStore) => reduxStore?.InvoiceInfoTableReducer?.orderData
  );
    ...
    let updateData = invoiceData.map((data) => {
      if (data.key === key) {
        return {
          ...
          subTotalPrice: Number.parseInt(
            data?.price * qty + data?.totalAddonsPrice
          ),
          amount: Number.parseInt(
            data?.totalAddonsPrice +
              (data?.price * qty -
                Number.parseInt(data?.perProductDiscount * qty))
          ),
        };
      } else {
        return {
   ...
   * created_at:- 22/08/2022 15:59:39
   * description :- A method is simply a “chunk” of code.
   */
  const getCustomerByType = async (value) => {
  ...
  const { currency_symbol, currency_position } = useSelector((store) => {
    ...
    settings.forEach((setting) => {
      ...
      if (setting?.name === "currency_position")
        currency_position = setting?.value;
    });
        ...
        <div className="mb-4">
          <div className="row mb-2">
              ...
              <div className="form-group mb-2">
                ...
                >
                  ...
                  <Modal.Body>
                    <form
                      action="#"
                      onSubmit={handleSubmit(newCustomerFormOnSubmit)}
                    >
                      <div className="mb-1 row gx-2">
                        <label
                          htmlFor="#"
                          className="col-sm-3 text-end col-form-label"
                        >
                          First Name
                        </label>
                        <div className="col-sm-9">
                          <input
                            ...
                            className={`form-control form-control-sm ${
                              errors?.first_name && "border-danger is-invalid"
                            }`}
                            id="#"
            ...
            <div className="col-xl-6">
              <div className="form-group mb-2">
                <label htmlFor="#" className="mb-1 form-label">
                  Waiter: <strong>{selectedWaiter?.label}</strong>
                </label>
                <Select
                  ref={waiterRef}
                  placeholder="Waiter ..."
          ...
          <p className="mb-0 d-flex flex-wrap gap-2 pb-1">
            ...
            {!_.isUndefined(selectedCustomerName?.first_name) && (
              <>
                <small>
                  ...
                  {`${selectedCustomerName?.first_name}`}
                </small>
              </>
            )}
            {!_.isUndefined(selectedCustomerName?.cust_phone) && (
              <>
                <small>
                  <strong>Phone</strong> :{" "}
                  {`${selectedCustomerName?.cust_phone}`}
                </small>
              </>
            )}
            ...
            {!_.isUndefined(selectedWaiter?.emp_his_id) && (
              <>
                <small>
                  <strong>Waiter Name</strong> : {selectedWaiter?.first_name}{" "}
                  {!_.isUndefined(selectedWaiter?.middle_name) &&
                    selectedWaiter?.middle_name}{" "}
                  {selectedWaiter?.last_name}
                </small>
              </>
            )}
          </p>
                  ...
                  return (
                    <tr key={key}>
                        ...
                        <div className="btn-group" role="group">
                          <button
                            ...
                            onClick={() => {
                              let obj = {
                                key,
                                id: item_code ?? productId,
                                item,
                                event_type,
                              };
                              editInvoiceProduct(obj);
                            }}
                          >
                            <MdOutlineResetTv />
                          </button>
                      ...
                      <td className="text-nowrap">
                        {currency_position === "prefix" && currency_symbol}{" "}
                        {price}{" "}
