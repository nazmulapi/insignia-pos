...
const InvoiceInfo = ({ getTotalInfo, editInvoiceProduct }) => {
	...
	const [selectedCustomerType, setSelectedCustomerType] = useState({ id: 4, value: 'hotel', apiKeyWord: 'hotel', label: 'Hotel Customer' });
	 ...
	 */
	const invoiceInfoTableData = useSelector((reduxStore) => reduxStore?.RoomInvoiceInfoTableReducer?.invoiceTableData);
	...
	const onQuantityChange = (key, e) => {
		...
		dispatch({ type: 'UPGRADE_ROOM_INVOICE_DATA', payload: updateData });
	...
	const removeInvoiceData = (key) => {
		if (key) {
			dispatch({ type: 'REMOVE_ROOM_INVOICE_DATA', payload: key });
			scannerBeepsSoundPlay();
		}
	};
	 ...
	 */
	useEffect(() => {
		customerTypeOnChangeHandler(selectedCustomerType);
		getAllTables();
		getAllWaiter();
	}, []);
								...
								{invoiceData.map((pro) => {
									...
									return (
										<tr key={key}>
											<td>
												<div className="btn-group" role="group">
													<button
														...
														onClick={() => {
															let obj = { key, id: item_code ?? productId, item, event_type };
											...
											<td>
												{currency_position === 'prefix' && currency_symbol} {price} {currency_position === 'suffix' && currency_symbol}
											</td>
											<td>
												{currency_position === 'prefix' && currency_symbol} {subTotalPrice} {currency_position === 'suffix' && currency_symbol}
											</td>
											<td>
												{currency_position === 'prefix' && currency_symbol} {totalDiscount} {currency_position === 'suffix' && currency_symbol}
											</td>
											<td>
												{currency_position === 'prefix' && currency_symbol} {amount} {currency_position === 'suffix' && currency_symbol}
											</td>
										</tr>
									);
								})}
